...
function _bash-it-is-partial-match() {
	local component="${1?}"
	local term="${2?}"
	_bash-it-component-help "${component}" | "${BASH_IT_GREP:-egrep}" -E -i -q -- "${term}"
}
	...
	if [[ "${#matches[@]}" -gt 0 ]]; then
		...
		for match in "${matches[@]}"; do
			...
			if [[ "${action}" == "${compatible_action}" ]]; then
				...
				modified=1
				# shellcheck disable=SC2034 # no idea if this is needed or not but...
