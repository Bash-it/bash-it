...
function __bp_precmd_invoke_cmd() {
    ...
    for precmd_function in "${precmd_functions[@]}"; do
        ...
        if type -t "$precmd_function" 1>/dev/null 
     then
...
function __bp_in_prompt_command() {
    ...
    for command in "${prompt_command_array[@]:-}"; do
        ...
        if [[ "$trimmed_command" == "$trimmed_arg" ]] 
     then
            return 0
        fi
    done
...
function __bp_preexec_invoke_exec() {
    ...
    if  __bp_in_prompt_command "${BASH_COMMAND:-}" 
     then
    ...
    for preexec_function in "${preexec_functions[@]:-}"; do
        ...
        if type -t "$preexec_function" 1>/dev/null 
     then
            __bp_set_ret_value ${__bp_last_ret_value:-}
            # Quote our function invocation to prevent issues with IFS
            "$preexec_function" "$this_command"
            preexec_function_ret_value="$?"
            if [[ "$preexec_function_ret_value" != 0 ]] 
     then
